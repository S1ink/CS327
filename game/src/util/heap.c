...
void print_heap_node(
    HeapNode *n, unsigned indent,
    char *(*print)(const void *v) )
{
    HeapNode *nc;

    printf("%*s%s\n", indent, "", print(n->datum));
    if(!(nc = n->child)) return;
...
void heap_node_delete(Heap *h, HeapNode *hn)
{
    HeapNode *next;
...
static void heap_link(Heap *h, HeapNode *node, HeapNode *root)
{
...
static void heap_cut(Heap *h, HeapNode *n, HeapNode *p)
{
    if(!--p->degree)
    {
        p->child = NULL;
...
static void heap_cascading_cut(Heap *h, HeapNode *n)
{
    HeapNode *p;
...
{
    if(h->compare(n->datum, v) <= 0) return 1;
